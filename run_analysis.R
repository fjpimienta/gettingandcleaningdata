...
setwd("/Users/familiaperezfuentes/GitHub/Data/")
path <- getwd()
d <- 'UCI HAR Dataset'
features <- read.table(file.path(path, d, 'features.txt'))
## read in the test data
test_x <- read.table(file.path(path, d, 'test', 'X_test.txt'))
test_y <- read.table(file.path(path, d, 'test', 'y_test.txt'))
test_subjects <- read.table(file.path(path, d, 'test', 'subject_test.txt'))
train_x <- read.table(file.path(path, d, 'train','X_train.txt'))
train_y <- read.table(file.path(path, d, 'train','y_train.txt'))
train_subjects <- read.table(file.path(path, d, 'train','subject_train.txt'))
...
## verification, with data from the middle of the matrix
